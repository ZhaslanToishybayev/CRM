...
class SupabaseTaskDataSource implements TaskDataSource {
  ...
  Future<TaskModel?> getTaskById(String taskId) async {
    ...
    try {
      // Don't filter by user_id - RLS policies will handle it
      final response = await _client
          .from('tasks')
          .select()
          .eq('id', taskId)
  ...
  Future<TaskModel> createTask(TaskModel task) async {
    ...
    // Don't manually set user_id - RLS policies will handle it via auth.uid()
  ...
  Stream<List<TaskModel>> watchTasks() {
    ...
    // Use the stream API from Supabase which handles RLS automatically
    return _client
        .from('tasks')
